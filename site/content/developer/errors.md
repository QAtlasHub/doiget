...
| Persona | Surface |
|---|---|
| Agent (MCP) | Structured, never throws. On failure: `{ ok: false, error: { code, message, disposition, denial_context? } }`. `disposition` is present on every failure that carries a structured `error` OBJECT, and is the field to branch a retry on — see §2 and ADR-0055. **Four tools do not yet emit that object at all** and instead put a bare string in `error`: `doiget_resolve_citation`, `doiget_batch_resolve_citations`, `doiget_tag` and `doiget_annotate`. That set is pinned by `every_bare_string_error_site_is_a_known_one` so it can shrink but not grow; this document said "ALWAYS present" before that was checked, which was the same overclaim it exists to prevent. It is derived from `code` by `ErrorCode::disposition`, never hand-written per call site. `remediation` IS carried here when the failure has a `denial_context`, from the same `remediation::for_denial` the blocked leg and the CLI `= help:` block use — this document previously said it was not, which meant the one field naming the fix was present when a call succeeded with a blocked leg and absent when the call actually failed (#506). It is omitted when there is no named fix rather than emitted empty. `attempts` remains an `ok: true` field. A blocked PDF leg is still an `ok: true` result with a failed leg, not an `ok: false` call, and it carries `pdf.remediation` plus `pdf.disposition`. |
