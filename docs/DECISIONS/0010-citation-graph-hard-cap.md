...
expand_citation_graph applies library const hard-caps regardless of caller-supplied parameters. truncated: true is set in the response when a cap is hit. Cycle detection via a visited set is mandatory. TDM sources are not used during citation graph expansion.
...
To revise this decision, write a new ADR with Status: Accepted and Supersedes: 0010,
